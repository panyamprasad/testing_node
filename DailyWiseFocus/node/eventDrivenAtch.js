...
// How to implement EDA in Node.js?
    // 1. Previosly we are using the eventEmitter from Nodejs.
    // 2. In this model, the eventBus publish an event and the subscribers are react on it.
    // 3. There is no direct dependency between the publisher or subscribers - It is lossly coupled.
    // 4. And it will not work acrros different services or servers. It will work single nodeJs process. It will useful for small applications.

    // 1. In Modern Architectures we are using cloud based EDA.
    // 2. NodeJs application integrate with aws services like SNS/SQS, lambda, eventBridge like.
    // 3. This flow application release an event to SNS or evntBridge. This event routed to different subscribers like lambda, sqs or other services.
    // 4. Each subscriber will work independently.
    // 5. This make the system scalable and fiexible and loosly coupled.
//--- ---------------------------------------------------------------------------------
...
// How EDA is used in microservices architecture?
    // 1. Basically Event driven architecture means system design patteran, used this we communicate the components, modules throguht the event, instend of direct connecct with api.
	// 2. MicroServices means we can build multiple services independentelly, each and every service can connect throught the Rest api's. So it is tightly copuled, because if any service down it will impact other.
