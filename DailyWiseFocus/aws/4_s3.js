...
// 1. What is Aws S3?
    // S3 means simple storage service, use this we can store and retrive the data at any time and any where.
    // It is similar to hardDisk, we can store files like images, vidoes and audios.
    // it is durabule it means the data is safe even if anything fail in aws services.
    // S3 is scalable, secure and cheep.

// 2. What is S3 buckek and object?
    // Bucket means folder, we can create any number floders in the s3, each bucket name is unique.
    // Object means file, that is stored in the bucket.

// 3. What are the S3 stroage classes?
    // S3 provide different stroage classes based on the purpose we can use it.
    // Standard -- For everyday use we can use this.
                // It is very cost compare to remaing.
                // And it is very fast
                // Data is store in multiple palces for safety purpose.
    ...
    // Standard IA -- In frequently used
                ...
                // it is fast
                // Store backup files, monthlly reports, old but important files.
   ...
   // 🧠 Real Example from a Company:
                // Imagine a banking application:
                    ...
                    // Monthely repots -- Standard-IA
                    // Daily transacation logs -- intelligent-Tiering
                    // Old files -- Glacier
  //---------------------------------------------
...
// 4. How do you control access to s3?
    ...
    // Presigned Url: This are temparory url use this to read and write the files.

// 5. Explain S3 Versioning?
    // Versioning means backup. If any file delete or update we can store the privious file or delete file in backup.
...
// 6. How do you handle large files in S3?
    ...
    // To overcome this problam, we use the multipart upload.
    // Multipart upload means:
            // Instend of load the full file at once, we can split the files in small chucks.
            // These chucks are uploaded in parllel.
            // If any think is fail, it will not impact remaing , only that chuck will reupload.
    ...
    // How will do this process:
        ...
        // Upload - It will handle the chuck upload. This class automaticaly split the file into chucks and process them to upload.
...
// 7. What is S3 Event Notification?
    ...
    // Based on the configuration we have tirgger the services.
    // When we upload the file/image to s3, we have to create the event notificaiton in the s3.
    // Based on the requirement we configure it like lambda/sns/sqs.

// 8. How do you optimize S3 for performance??
    // Use the multipart uplaod for big files.
