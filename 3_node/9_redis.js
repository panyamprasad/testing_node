...
//         2. It is in-memory database, data is storage in Ram, not in disk.
//         3. Used for:
...
//         1. When we fetch the data from db or external api, the data will expiry shortly. 
//         2.The expire time will add when store the data.
//         3. So we can store it in Redis, and from the next requests will get it from redis. 
//         4. If the data is not available in redis then again api call will happen.
//         5. It mean Redis is temporary storage, and it is very fast compare to db.
