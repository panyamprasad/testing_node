...
// Why use CloudFormation?
    // 1. We don't have to click and create the resources manually in aws console.
    // 2. Same template we can use multiple environments like dev, qa, stage and prod.
    // 3. We can store infrastructure in the git hub based on version.
    // 4. And cloudFormation understand the order of creating resources.
    //-----------------------------------------------------

// When will use CloudFormation?
    // 1. When we need multiple resources then will follow the cloudFormation instead of manual creation.
    // 2. When we need automatic deployment using ci/cd
    // 3. When we need same infrastructure in all environments.
    //-----------------------------------------------------

// Why do we use cloudformation instead of manual creation?
    // It is automatic process, reduce the errors, version controlling, and environment consistent.
    //-----------------------------------------------------
...
// Stacks and template?
    ...
    // Stack:
        ...
        // 2. When we deploy the template, cloudformation creates a stack that contained all defined services like lambda, apigateway, s3 etc.
    //-----------------------------------------------------
...
// Lambda deployment with serverless framework?
    // 1. If we have the serverless project, when we deploy the code using serverless deploy, it actually create the cloudformation template and it's includes all the resources details.
...
// Rollback after deployment failure?
    ...
    // 2. For example in deployment if any table creation is failed, so cloudformation automatically rollback all successful resources and revert the previous stable version.
    //-----------------------------------------------------

// What is different between serverless.yaml and cloudformation.yaml?
    // Serverless.yaml:
        // 1. Serverless yaml is the part of in serverless framework, using this we can define the all the configurations.
        // 2. Like Lambda functions, IAM roles & polices, Events like APIgateway, SQS etc, environment variable.
    
    // CloudFormation:
        // It is pure AWS Cloudformation, used to define the infra directly without any extra framework, we must specify the all the resource properties.
