// 1. What is NodeJs?
        // NodeJs is a runtime environment, that uses a V8 javaScript engine. It allows us to use javaScript for backend development.
        // NodeJs is single threaded and event-driven, and it is used event loop to handle the asynchronous operations. In even loop it will handle multiple phases like settimeout, setImmediate, promises and async/await.
        // NodeJs is open-source and free to use, with nodeJs we can build serverless architectures, microservices and RestFul API's.
        // It's simple and secure and easy to learn, and it has the huge ecoSystem for ready to use many inbuilt packages and modules.

// 2. What is difference between Node and JavaScript?
        // NodeJs is runtime environment that allows JavaScript to run at outside of the browser, using V8 engine. With Node we can use JavaScript to build backend api's, microservices and serverless architecture.
        // JavaScript is a programming/Scripting language, it was using to develop web api's and run in browsers.
...
// 3. Can you explain the working of nodeJs?
        ...
        // Node is single threaded and using the event-driven, this means it is single thread, but it will handle the multiple requests at the same time.
...
// 4. Why nodeJs Single threaded?
        // Node is single threaded because it's designed to handle the multiple tasks effectively instead of blocking the tasks.
        // Instead of blocking the main thread it will use the event loop to handle the asynchronous operations.
...
// 6. What is Synchronous and Asynchronous programming?
        ...
        // Asynchronous:
                // Asynchronous is non-blocking and single threaded, it will handle multiple tasks same time.
                // It will run the tasks at the background using the event loop.
                // like Reading the files, getting the data from db, uploading the file like this processes will handle the event loop at the backend using the promises, async/await calls.
        
// 8. How to achieve nodeJs Asynchronous Programming?
        // Using the eventloop node will achieve the asynchronous programming, in event loop there different phases to handle these operations. So with out blocking the code, use event loop complete the asynchronous operations.
        // Need more explanation will do it.....


// 9. What is event loop in NodeJs, and how does it work?
        // Basically nodeJs is the single threaded, but using the event loop it will handle the asynchronous tasks with out blocking.
        ...
        //Followup question: If you call an API It will take 5s, does nodeJs block?
                // NO, it will not block it will execute the other task simantanisoly this task will execute in callback and return the response.

// 10. What is callback hell and what are the methods to avoid it?
        // Callback is the one of the way to handle the asynchronous operations in nodeJs.
        // But when we use this there are multiple callbacks will create the nested callbacks, due to this callback hell will happen.
        // It becomes hard to read the code, hard to maintain, and debug.
        // To overcome this node is introduced the promises chain process after this Async/Await process.

// 11. What are promises in nodeJs?
        // Promises is one of the way to handle the asynchronous operations in cleaner and more readable way.
...
// 12. How can you use Async/await in nodeJs? Provide
        ...
        // Await hold the execution until the promises is complete the execution or rejected. once it's done it will give the response or error.

// 13. What is Package.json and where is it used?
        // Package.json is main file to handle the nodeJs project. It includes all the dependencies and scripts, project metadata it contains name, version and description.
...
// 15. What is middleware in NodeJs, Why it is used?
        ...
        // There are different types of middlewares are there
                ...
                // 3. Custom-middleware: User can create there own middlewares purpose of handle the authentication and logging. (res, req, next)
...
// 16. What is the purpose of modules exports in Nodejs?
        // Modules are nothing but it's collection of file like js and ts and any other files. It includes variables, functions, classes and json file. If we want wherever we required it we can export and import this file in project. 
        // Main use of modules are reusability.
        // There are different type of modules are there.
                ...
                // Third party modules: express, axios, lodash. Use the npm we can install this modules.

// 17. What is different between NodeJs and ExpressJs?
        // NodeJs: NodeJS is the runtime environment using the v8 javaScript engine and run it in browser.
        // Expressjs: ExpressJs is framework in node, it will provide the structure, organization, and common utilities to develop for web api's in cleaner and maintainable way.

// 18. What is event-driven programming?

// 19. What is the Role of the events in node? How can you handle events in node?

// 20. Difference between setTimeout() and setImmediate()?
...
// 21. Explain the nodeJs web application architecture?
        // When the client (Browser/mobile app) sends the http request.
        // The server or express receive the request and start the process.
        // The middleware run the request and verify it before reach the route. (logging/authentication)
        // The route will identify the url or path, it will start execute the function.
        // Based on the business logic it will connect with backend services and get back the response.
        // Server giving back the response to client.

// 22. What are the types of streams available in NodeJs?
        // Stream is nothing but how to handle the file. Instead of loading the full data using the stream we can load chunk by chunk.
...
// 24. What are some of the most commonly used Libraries in NodeJs?
        // ExpressJs: ExpressJs is a flexible nodeJs web application framework. It will provide a huge features to develop the web and mobile applications.
        // Mongoose: Mongoose also a nodeJs web application framework, it makes easy to connect an application to database.
//-------------------------------------------------------------------

// 25. How to handle the asynchronous events in Node?
...
// 26. How do you manage error handling using nodeJs?
...
// 27. How do you secure NodeJs Application?
        ...
        // Always do the input validation
        // Use the hashing algorithm for storing the sensitive information.
        // Always use the https and secure headers.
