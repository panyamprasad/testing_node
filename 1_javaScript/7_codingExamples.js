...
function factorial(n) {
    // If the above condition is not mention it will go infinity loop..
    // Will get the maximum call stack size error will come. 
    // It is Base case. It will stops the recursive calls and return the value.
    if (n === 0 || n === 1) {
        return 1;
    }
...
//---> Check Prime Number
// Here we are using the sqrt. using this to reduce the number of loop iterations to make program faster.
// I use i <= Math.sqrt(num) to improve performance because a number's factors always appear in pairs, so checking beyond the square root is unnecessary.
function isPrime(n) {
    if (n <= 1) {
        return false;
    }
...
// Here we are using the seen created in the outer function.
// After Outer Function execution is complete still we are accessing the seen inner function.
