...
// 		2. For example if we have the common utilities and dependencies in multiple lambda functions, we can kept this in single file or folder we can use this across all the files, this is called layers. 
...
// 			3. This way I can find and fix  the issues quickly.
	
// ---> 3. How node will work? if it is single threaded?
// 		1. NodeJs is single threaded, but it will handle the asynchronous operations without blocking the code using the event loop.
// 		2. In the event loop it have the multiple phases to handle the asynchronous operations. The phases are like below:
// 			timer phase: This phase event loop execute the setTimeout and setIntervals.
...
// 			microTasks: This phase promises, async/await functions will execute.
